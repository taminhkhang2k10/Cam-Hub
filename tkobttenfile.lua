...
local function NewLine(color, thick)
    ...
    l.Visible = false; l.Color = color or Color3.new(1,1,1)
    l.Thickness = thick or 1; l.ZIndex = 4
    return l
end

local function NewText(color, size)
    local t = Drawing.new("Text")
    t.Visible = false; t.Color = color or Color3.new(1,1,1)
    t.Size = size or 13; t.Font = Drawing.Fonts.UI
    t.Outline = true; t.OutlineColor = Color3.new(0,0,0); t.ZIndex = 6
    return t
end

local function NewQuad(color)
    local q = Drawing.new("Quad")
    q.Visible = false; q.Filled = true
    q.Color = color or Color3.new(1,1,1)
    q.Transparency = 1; q.ZIndex = 3
    return q
end
...
local function UpdateESP(esp, char, color, label)
    ...
    if showBox and Config.BoxFillOpacity > 0 then
        esp.BoxFill.PointA = b.tl; esp.BoxFill.PointB = b.tr
        esp.BoxFill.PointC = b.br; esp.BoxFill.PointD = b.bl
        esp.BoxFill.Color  = color
        esp.BoxFill.Transparency = 1 - Config.BoxFillOpacity
    end

    -- Box
    esp.BoxTop.Visible = showBox; esp.BoxBot.Visible = showBox
    esp.BoxLeft.Visible = showBox; esp.BoxRight.Visible = showBox
    if showBox then
    ...
    if showName then
        esp.Name.Text = label; esp.Name.Color = color
        esp.Name.Position = Vector2.new(b.midX, b.topY - 16); esp.Name.Center = true
    end
    ...
    if showDist then
        esp.Dist.Text = "[" .. dist .. "m]"; esp.Dist.Color = Config.TextColor
        esp.Dist.Position = Vector2.new(b.midX, b.botY + 2); esp.Dist.Center = true
        esp.Dist.Size = math.clamp(11 - dist/150, 7, 11)
    end

    -- HP Bar
    local showHP = Config.ESPEnabled and Config.HealthEnabled
    esp.HPBg.Visible = showHP; esp.HPFill.Visible = showHP
    if showHP then
        local hpRatio = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
        local barX = b.tl.X - 5
        esp.HPBg.From = Vector2.new(barX, b.topY); esp.HPBg.To = Vector2.new(barX, b.botY)
        ...
        esp.HPFill.Color = Color3.new(math.clamp(2*(1-hpRatio),0,1), math.clamp(2*hpRatio,0,1), 0)
    end
...
RunService.RenderStepped:Connect(function()
    ...
    for char, esp in pairs(espObjects) do
        if not seen[char] then RemoveESP(esp); espObjects[char] = nil end
    end
end)
...
VisualTab:AddToggle("Box", true, function(v) Config.BoxEnabled = v end)
VisualTab:AddToggle("Tên (Name)", true, function(v) Config.NameEnabled = v end)
VisualTab:AddToggle("Thanh máu (HP)", true, function(v) Config.HealthEnabled = v end)
VisualTab:AddToggle("Khoảng cách", true, function(v) Config.DistEnabled = v end)
VisualTab:AddToggle("Tracer", true, function(v) Config.TracerEnabled = v end)

VisualTab:AddSection("Màu sắc")

VisualTab:AddColorPicker("Màu Player", Config.PlayerColor, function(c) Config.PlayerColor = c end)
VisualTab:AddColorPicker("Màu NPC", Config.NPCColor, function(c) Config.NPCColor = c end)
...
VisualTab:AddSlider("Tầm nhìn tối đa (m)", 100, 2000, 1000, function(v) Config.MaxDist = v end)
VisualTab:AddSlider("Kích thước Box", 50, 200, 100, function(v) Config.BoxScale = v / 100 end)
VisualTab:AddSlider("Độ dày viền Box", 1, 5, 2, function(v)
    Config.BoxThickness = v
    for _, esp in pairs(espObjects) do
        esp.BoxTop.Thickness = v; esp.BoxBot.Thickness = v
        esp.BoxLeft.Thickness = v; esp.BoxRight.Thickness = v
    end
end)
VisualTab:AddSlider("Độ tô kín Box (0-10)", 0, 10, 0, function(v) Config.BoxFillOpacity = v / 10 end)

-- ═══════════════════════════════════════════════
--  TAB: AIMBOT (coming soon)
-- ═══════════════════════════════════════════════
